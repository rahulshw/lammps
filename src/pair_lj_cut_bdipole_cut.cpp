...
void PairLJCutBDipoleCut::settings(int narg, char **arg)
{
  ...
  else cut_di_global = utils::numeric(FLERR,arg[1],false,lmp);
...
void PairLJCutBDipoleCut::coeff(int narg, char **arg)
{
  ...
  if (narg >= 5) cut_di_one = cut_lj_one = utils::numeric(FLERR,arg[4],false,lmp);
  if (narg == 6) cut_di_one = utils::numeric(FLERR,arg[5],false,lmp);
